...
//	});
	...
	//点击对应按钮进行渲染
//	eventUtil.addHandler(document.getElementById("showsortadd"),'mouseover',function(){
//		getAllTopSort("add");
//	});
//	
//	eventUtil.addHandler(document.getElementById("showsortedit"),'click',function(){
//		getAllTopSort("edit");
//	});
}
/**
 ...
 * @returns
 */
function getAllTopSort(type){
	var showsort = document.getElementById("showsort"+type);
	ajaxGson("GET","./getalltopsortname.do",function(){
		if (xmlhttp.readyState==4 && xmlhttp.status==200){
			...
			for(var i=0;i<options.length;i++){
				...
				for(var o in options[i]){
					if(o == "sortname"){
						option.innerHTML = options[i][o];
					}
					if(o == "sortid"){
						option.value= options[i][o];
					}
				}
				showsort.appendChild(option);
			}
		}
	});
}
...
function submitadd(type){
	...
	if(fileform != null){
		fileform.append("editobj",string);
		ajaxGsonHasData("POST","./adminaddHasData"+type+".do",fileform,function(){
			if (xmlhttp.readyState==4 && xmlhttp.status==200){
				alert(xmlhttp.responseText);
				//刷新面板
				flashObjList(type);
	...
	for(var i=0;i<inputs.length;i++){
		...
		if(inputs[i].name =="bookname"){
			...
			inputs[i].onblur = function(){
				if(regExp('^\\w+$|^[\u4e00-\u9fa5]+$|^\\w+[\u4e00-\u9fa5]+$|^[\u4e00-\u9fa5]+\\w+$','i',this.value)){
					this.title="";
					$('[name="bookname"]').tooltip('destroy');
					regExpsuccess(this);
